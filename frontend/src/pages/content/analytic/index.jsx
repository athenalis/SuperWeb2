...
export default function AnalyticContent() {
  ...
  const availableMonths = useMemo(() => {
    ...
    return Array.from(months).sort().map(m => {
      ...
      return {
        value: m,
        label: new Date(year, parseInt(month) - 1).toLocaleDateString("id-ID", {
          month: "long",
          year: "numeric"
        })
      };
  ...
  /* =========================
     CHART DATA - DENGAN FILTER & LINE KONTINYU
  ========================= */
  const chartData = useMemo(() => {
    ...
    // =============================
    // MODE 1: Semua Konten + Semua Bulan → BULANAN
    // =============================
    if (selectedContentType === "all" && selectedMonth === "all") {
      const monthly = {};
    ...
    // =============================
    // MODE 2: Semua Konten + Bulan tertentu → MINGGUAN
    // =============================
    if (selectedContentType === "all" && selectedMonth !== "all") {
      filtered = filtered.filter(item => {
        const d = new Date(item.end_date);
    ...
    // =============================
    // MODE 3: Content Type tertentu → HARIAN
    // =============================
    ...
    filtered.forEach(item => {
      ...
      if (!grouped[key]) grouped[key] = { views: 0, likes: 0 };
      grouped[key].views += Number(item.views);
      grouped[key].likes += Number(item.likes);
    });
    ...
    return {
      labels: sorted.map(([d]) => formatDateID(d, true)),
      views: sorted.map(([, v]) => v.views),
      likes: sorted.map(([, v]) => v.likes),
    };
  ...
  const validateInput = (name, value) => {
    if (!selectedContentPlatform) return "";
    if (value === "") return "";

    const platformReport = reports[selectedPlatform];
    if (!platformReport) return "";

    const contentTypeData = Object.values(platformReport.content_types || {}).find(
      ct => ct.content_platform_id === selectedContentPlatform
    );

    if (!contentTypeData || !contentTypeData.data || contentTypeData.data.length === 0) {
      return "";
    }

    const lastRecord = contentTypeData.data[contentTypeData.data.length - 1];

    if (name === "views" && Number(value) < lastRecord.views) {
      return "Jumlah Views tidak boleh kurang dari data sebelumnya";
    }
    if (name === "likes" && Number(value) < lastRecord.likes) {
      return "Jumlah Likes tidak boleh kurang dari data sebelumnya";
    }
  ...
  const tableData = useMemo(() => {
    let data = chartRaw.filter(c => c.platform_id === selectedPlatform);

    if (selectedContentType !== "all") {
      data = data.filter(c => c.content_platform_id === Number(selectedContentType));
    }
    ...
    // 🔽 INI KUNCI UTAMA — URUTKAN BERDASARKAN TANGGAL
    return [...data].sort((a, b) => new Date(a.end_date) - new Date(b.end_date));
  }, [chartRaw, selectedPlatform, selectedContentType, selectedMonth, selectedYear]);


  const validateDateRange = (startDate, endDate, isEdit = false, recordId = null) => {
  ...
  const handleEditChange = (name, value) => {
    if (name === "start_date" || name === "end_date") {
      ...
      setTimeout(() => {
        ...
        const dateErrors = validateDateRange(newStartDate, newEndDate, true, editData.id);
        setErrors(prev => ({
          ...prev,
          ...dateErrors,
        }));
  ...
  const handleUpdate = async () => {
    ...
    const dateErrors = validateDateRange(editData.start_date, editData.end_date, true, editData.id);
    if (dateErrors.start_date || dateErrors.end_date) {
      toast.error(dateErrors.start_date || dateErrors.end_date);
      return;
    }
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-8">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2">
          <Icon icon="mdi:filter-outline" className="text-lg text-slate-600" />
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
          <div>
            <label className="text-sm font-semibold text-slate-700 mb-2 block">Tipe Konten</label>
            <select
          ...
          <div>
            <label className="text-sm font-semibold text-slate-700 mb-2 block">Tanggal Mulai</label>
            <DatePicker
          ...
          <div>
            <label className="text-sm font-semibold text-slate-700 mb-2 block">Tanggal Akhir</label>
            <DatePicker
          ...
          <div>
            <label className="text-sm font-semibold text-slate-700 mb-2 block">Jumlah Views</label>
            <input
              type="text"
              name="views"
              disabled={isDisabled}
          ...
          <div>
            <label className="text-sm font-semibold text-slate-700 mb-2 block">Jumlah Likes</label>
            <input
              type="text"
              name="likes"
              disabled={isDisabled}
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 border-t-2 border-slate-200 bg-slate-50">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-4 py-2 border-2 border-slate-300 rounded-lg font-medium text-slate-700 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-100 transition-all duration-200"
                >
                  Sebelumnya
                </button>
                {Array.from({ length: reportTotalPage }, (_, i) => i + 1).map((p) => (
                  <button
                    ...
                    className={`px-4 py-2 border-2 rounded-lg font-medium transition-all duration-200 ${p === reportPage
      ...
      {isEditOpen && editData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl w-full max-w-[480px] shadow-2xl animate-in fade-in zoom-in duration-200">
            <div className="px-8 pt-8 pb-6 border-b-2 border-slate-200">
              <div className="flex items-center gap-3 mb-2">
                <div className="w-12 h-12 bg-blue-100 rounded-xl flex items-center justify-center">
                  <Icon icon="solar:pen-bold" className="text-2xl text-blue-900" />
                </div>
                <h3 className="text-blue-900 font-bold text-2xl">Edit Data</h3>
              </div>
              <p className="text-sm text-slate-500 ml-15">Perbarui data analitik konten Anda</p>
            </div>
            <div className="px-8 py-6 space-y-5">
              <div>
                <label className="text-sm font-bold text-slate-700 block mb-2">Tanggal Mulai</label>
                <DatePicker
                  selected={editData.start_date ? new Date(editData.start_date) : null}
                  onChange={(date) => {
                    const dateStr = toLocalDateString(date);
                    handleEditChange("start_date", dateStr);
                  }}
                  excludeDates={blockedDatesForEdit}
                  dateFormat="yyyy-MM-dd"
                  placeholderText="Pilih tanggal mulai"
                  className={`w-full border-2 rounded-xl px-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent
                    ${errors.start_date ? "border-red-500" : "border-slate-300"}`}
                />
                {errors.start_date && (
                  <p className="text-xs text-red-600 mt-1">{errors.start_date}</p>
              ...
              <div>
                <label className="text-sm font-bold text-slate-700 block mb-2">Tanggal Akhir</label>
                <DatePicker
                  selected={editData.end_date ? new Date(editData.end_date) : null}
                  onChange={(date) => {
                    const dateStr = toLocalDateString(date);
                    handleEditChange("end_date", dateStr);
                  }}
                  excludeDates={blockedDatesForEdit}
              ...
              <div>
                <label className="text-sm font-bold text-slate-700 block mb-2">Jumlah Views</label>
                <input
                  type="text"
                  value={Number(editData.views).toLocaleString("id-ID")}
                  onChange={(e) => handleEditChange("views", e.target.value.replace(/\D/g, ""))}
                  className={`w-full border-2 rounded-xl px-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent
                    ${errors.views ? "border-red-500" : "border-slate-300"}`}
                />
                {errors.views && (
              ...
              <div>
                <label className="text-sm font-bold text-slate-700 block mb-2">Jumlah Likes</label>
                <input
                  type="text"
                  value={Number(editData.likes).toLocaleString("id-ID")}
                  onChange={(e) => handleEditChange("likes", e.target.value.replace(/\D/g, ""))}
                  className={`w-full border-2 rounded-xl px-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent
                    ${errors.likes ? "border-red-500" : "border-slate-300"}`}
                />
                {errors.likes && (
            ...
            <div className="px-8 pb-8 space-y-3">
              <button
                ...
                className="w-full py-3.5 rounded-xl bg-blue-900 text-white text-lg font-bold hover:bg-blue-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 hover:shadow-lg"
              >
                {saving ? (
              ...
              <button
                ...
                className="w-full py-3.5 rounded-xl border-2 border-slate-300 hover:bg-slate-100 text-slate-700 text-lg font-semibold transition-all duration-200"
              >
                Batal
              </button>
            </div>
